#############################################################################################
[Legacy TE] Restrict Special Character Selection to L 1.1 + R-loader [PyotrLuzhin, Yohan1044, DukeItOut; R-loader by metanite64]
#
# 1.1: Re-added support for Wiimote and Wiimote+Nunchuk
#
# R-loader (metanite64): distinguishes L vs R rather than removing R entirely
#############################################################################################
# op li r0, 0x40 @ $8068480C	# Old version of code
HOOK @ $8068480C
{
	lwzx r0, r3, r0		# Original operation
	li r12, 0x20		# \ r12 checks for R button from r4, stores in r10
	and r10, r4, r12	# /
	cmpwi r10, 0		# \ if R was not held, skip writing to memory
	beq NotR		# /

	lis r9, 0x8000		# \
	ori r9, r9, 0x2960	# | store data to 0x80002960 if R was held (value doesn't matter)
	stw r10, 0(r9)		# /
NotR:
}
...
loc_0x74:
  li r0, 0x20;  and. r0, r3, r0;  b loc_0x88		# removed condition on this branch, disables R-alt costumes (thanks QuickLava)
