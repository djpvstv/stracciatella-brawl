...
####################
AltR/Z EX Fix 242 Variant [Desi, RedipsTheCooler]
####################
op lwz r7, 0x1314 (r28) @ $8084CFB8 # Load pointer to ".pac" string!
op lwz r6, 0x1318 (r28) @ $8084CE28 # Load pointer to "Dark" string!
op lwz r7, 0x1314 (r28) @ $8084CE30 # Load pointer to ".pac" string!
...
.alias PacPathPtrArray          = 0x817B8350  	# <- Changed for REX
...
HOOK @ $80946424               # [in  "processBegin/[stLoaderPlayer]/st_loader_player.o" @ $80946004, Ghidra: $809543B0]
{
  ...
  lwz r9, 0x1310(r4)                     # While we've got this address, also grab and stash the ".pac" path string for later! ####### <- CHANGED FOR REX
...
###############################################################
Holding X/Y Automatically Scrolls Through Costumes [Exul Anima]
###############################################################
...
HOOK @ $8069BE18
{
  ...
  word 0x00000000         #  \
  word 0x00000000         #   \
  word 0x00000000         #    > Persistent storage of saved timer states, one for each player.
  word 0x00000000         #   /
bltrick:                  #  /
  mflr r12                # /
  ...
  lwz r29, 0x14(r1)       # \
  andi. r11, r29, 0x400   #  > If the button was just pressed, initialize the timer and register the input.
  bne initScroll          # /
  lwz r29, 0xC(r1)        # \
  andi. r11, r29, 0x400   #  > Test if the button is continuing to be pressed.
  beq %END%               # /
  mulli r0, r25, 0x4      # \
  lwzx r11, r12, r0       #  > Check if player's scroll timer has reached the threshold.
  cmpwi r11, 0x8          # /
  blt incrementTimer      # If the timer has not yet reached the threshold, increment the player's timer without registering the input.
  li r11, 0x0             # \ Else reset the timer and allow the button input to be processed.
  stwx r11, r12, r0       # / The input will be allowed to go through once every few frames, allowing for automatic scrolling.
  b %END%
incrementTimer:
  addi r11, r11, 0x1      # \ Increment the player timer.
  stwx r11, r12, r0       # /
  li r29, 0x0             # Nullify the input.
  b %END%
initScroll:
  mulli r0, r25, 0x4      # \
  li r11, -0xC            #  > Give additional frames upon initialization for delay in scrolling start.
  stwx r11, r12, r0       # /
}
...
bltrick:                  #  /
  ...
  lwz r29, 0x14(r1)       # \
  andi. r11, r29, 0x800   #  > If the button was just pressed, initialize the timer and register the input.
  bne initScroll          # /
  lwz r29, 0xC(r1)        # \
  andi. r11, r29, 0x800   #  > Test if the button is continuing to be pressed.
  beq %END%               # /
  mulli r0, r25, 0x4      # \
  lwzx r11, r12, r0       #  > Check if player's scroll timer has reached the threshold.
  cmpwi r11, 0x8          # /
